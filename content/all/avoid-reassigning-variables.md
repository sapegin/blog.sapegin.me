...
And most of the time we can avoid both. Let’s start with reassigning and come back to mutation in the [next chapter](/all/avoid-mutation/).
...
Unfortunately, JavaScript has no true constants, and _mutation_ is still possible even when we define a variable with the `const` keyword. We’ll talk about mutations in [the next chapter](/all/avoid-mutation/).
